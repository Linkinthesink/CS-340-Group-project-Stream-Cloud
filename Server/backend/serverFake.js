...
app.get('/tracks-availability/:id', async (req, res) => {
    try {
        const id = req.params.id;
        const query = `SELECT t.trackID, t.trackTitle, t.releaseDate,
                      p.platformID, p.platformName, p.country,
                      ta.url
                   FROM tracks t
                   JOIN trackAvailability ta ON t.trackID = ta.trackID
                   JOIN platforms p ON ta.platformID = p.platformID
                   WHERE t.trackID = ?;`;
        const [rows] = await db.query(query, [id]);
        console.log(rows);
        res.status(200).json({ track: rows });
